...
:doc:`Regions <objects/region>` are areas that will affect the Pogo Dude while they are in it. You can create a region from the regular add menu under "PogoBlend". Regions have types that will have diffrent effects.
...
To make a custom map there is 3 required objects. A spawn postion, a start line, and a progress path. I reccomend using the provided application template, as new files using it will automatically create these.
...
I highly recommend you check out the other guides mentioned here on this page.
